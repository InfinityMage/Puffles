...
module.exports = {
    ...
    dev: false,
    aliases: [],
    examples: ["`punch @Vert3xo` : punches the person with the tag @Vert3xo"],
    ...
    async execute(message, args, client) {
        if (
            ...
            await message.channel.send(
                complexError("You can not punch yourself.")
            );
            return;
        }
        if (
        ...
        const url = fetch("https://neko-love.xyz/api/v1/punch")
            .then((res) => res.json())
            .then((json) => json.url);
        const progressEmbed = new discord.MessageEmbed()
            ...
            .addField(
                "**✊ Punch**",
                `${message.author.username} punched ${args[0]}`
            )
            .setImage(await url);
